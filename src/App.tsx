...
import { nvivoLessonsData } from './data/nvivoLessonsData';
import type { NvivoSection, NvivoLesson } from './data/nvivoLessonsData';
...
const COURSE_COVERS = {
  ...
  nvivo: "https://images.unsplash.com/photo-1432888498266-38ffec3eaf0a?auto=format&fit=crop&w=800&q=80"
};
...
export function App() {
  ...
  // NVivo track completion states
  const [completedNvivoLessons, setCompletedNvivoLessons] = useState<string[]>(() => {
    const saved = localStorage.getItem('debug_society_completed_nvivo_lessons');
    return saved ? JSON.parse(saved) : [];
  });

  // NVivo Lesson Immersive Reader State
  const [selectedNvivoLesson, setSelectedNvivoLesson] = useState<NvivoLesson | null>(null);
  const [selectedNvivoSection, setSelectedNvivoSection] = useState<NvivoSection | null>(null);
  ...
  useEffect(() => {
    ...
    const days = Math.max(1, Math.ceil((completedLessons.length + completedTypescriptLessons.length + completedRQualitativeLessons.length + completedNvivoLessons.length) / 2));
  ...
  }, [selectedLesson, selectedSqlLesson, selectedFastApiLesson, selectedExpressLesson, selectedPythonLesson, selectedTypescriptLesson, selectedRQualitativeLesson, selectedNvivoLesson]);
  ...
  const handleMarkNvivoLessonComplete = (lessonId: string) => {
    if (completedNvivoLessons.includes(lessonId)) {
      setCompletedNvivoLessons(completedNvivoLessons.filter(id => id !== lessonId));
      playSynthesizedSound('reset');
    } else {
      setCompletedNvivoLessons([...completedNvivoLessons, lessonId]);
      playSynthesizedSound('success');
      triggerConfetti();
    }
  };
  ...
  const handleAuthenticateUser = async (email: string, isSignUp: boolean, fullName?: string) => {
    ...
    setCompletedNvivoLessons(profile.completedNvivoLessons || []);
  ...
  const totalNvivoLessons = nvivoLessonsData.flatMap(s => s.lessons).length;
  const completedNvivoCount = completedNvivoLessons.length;
  const nvivoProgressPercent = totalNvivoLessons ? Math.round((completedNvivoCount / totalNvivoLessons) * 100) : 0;

  const totalAll = totalLessons + totalSqlLessons + totalFastApiLessons + totalExpressLessons + totalPythonLessons + totalTypescriptLessons + totalRQualitativeLessons + totalNvivoLessons;
  ...
  if (completedLessons.length + completedSqlLessons.length + completedFastApiLessons.length + completedExpressLessons.length + completedPythonLessons.length + completedTypescriptLessons.length + completedRQualitativeLessons.length > 0) {
    notifications.push({
      ...
      detail: `${completedLessons.length} React • ${completedSqlLessons.length} SQL • ${completedFastApiLessons.length} FastAPI • ${completedExpressLessons.length} Express • ${completedPythonLessons.length} Python • ${completedTypescriptLessons.length} TypeScript • ${completedRQualitativeLessons.length} R Qualitative • ${completedNvivoLessons.length} NVivo`,
      kind: 'streak'
    });
  }
  ...
  return (
    <div className={`app-frame theme-${devTheme}`}>
      ...
      <main className="main-content">
          ...
          <div>
            ...
            <div style={{ marginBottom: '32px' }}>
              ...
              <div className="courses-grid-view">
                ...
                {/* NVivo 14 Course Card */}
                <div className="course-card-classic">
                  <div className="course-cover">
                    <img src={COURSE_COVERS.nvivo} alt="NVivo 14 course cover" />
                  </div>
                  <div className="course-details">
                    <h5>NVivo 14: Complete Practical Course</h5>
                    <div className="lessons-count-meta" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                      <RiBarChart style={{ color: 'var(--text-muted)' }} />
                      <span>lessons : {totalNvivoLessons} (Full Syllabus)</span>
                    </div>
                    <div className="instructor-meta">
                      <img src="https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?q=80&w=256&auto=format&fit=crop" alt="Instructor avatar" />
                      <span>The Debug Society</span>
                    ...
                    <div className="progress-info-row">
                      <span>{nvivoProgressPercent}% Complete</span>
                    </div>
                    <div className="card-progress-track">
                      <div className="card-progress-fill-val" style={{ width: `${nvivoProgressPercent}%` }}></div>
                    </div>
                    <button className="card-action-btn-classic" onClick={() => {
                      setActiveTrack('nvivo');
                      setActivePage('react-path');
                      playSynthesizedSound('success');
                    }}>
                      {nvivoProgressPercent === 100 ? 'Review Path' : 'Resume Path'}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {/* NVIVO TRACK VIEW */}
        {activePage === 'react-path' && activeTrack === 'nvivo' && (
          <div>
            <div className="module-view-header">
              <div>
                <h2>The Debug Society — NVivo 14: Complete Practical Course</h2>
                <p>From raw transcripts to published findings. Master CAQDAS, inductive coding, thematic analysis, queries, and qualitative reporting.</p>
              </div>
              <div className="module-completion-badge">
                {nvivoProgressPercent === 100 ? (
                  <span className="badge-unlocked"><RiAward style={{ marginRight: '6px' }} /> Graduated from NVivo 14 Track</span>
                ) : (
                  <span><RiAward style={{ marginRight: '6px' }} /> Grad. Badge Locked ({completedNvivoCount}/{totalNvivoLessons} Lessons)</span>
                )}
              </div>
            </div>

            <div className="accordion-wrapper">
              {nvivoLessonsData.map((section, sIdx) => {
                const isExpanded = expandedSectionId === section.sectionId;
                const completedInSection = section.lessons.filter(l => completedNvivoLessons.includes(l.id)).length;
                const sectionPercent = Math.round((completedInSection / section.lessons.length) * 100);
                  ...
                  <div key={section.sectionId} className={`accordion-item ${isExpanded ? 'expanded' : ''}`}>
                    ...
                    {isExpanded && (
                      <div className="accordion-body">
                        <div className="lessons-list-group">
                          {section.lessons.map((lesson, lIdx) => {
                            const isCompleted = completedNvivoLessons.includes(lesson.id);
                            let statusTag = <span className="lesson-badge-tag locked">Locked</span>;

                            if (isCompleted) {
                              statusTag = <span className="lesson-badge-tag completed">Completed</span>;
                            } else {
                              const allPriorCompleted = nvivoLessonsData
                                .slice(0, sIdx + 1)
                                .flatMap((s, sI) => sI < sIdx ? s.lessons : s.lessons.slice(0, lIdx))
                                .every(l => completedNvivoLessons.includes(l.id));
                            ...
                            return (
                              <div
                                ...
                                className={`lesson-row ${isCompleted ? 'completed-lesson-item' : ''}`}
                                onClick={() => {
                                  setSelectedNvivoLesson(lesson);
                                  setSelectedNvivoSection(section);
                                  playSynthesizedSound('success');
                                }}
                              >
                                <div className="lesson-title-side">
...
{/* ── ADMIN PANEL ── */}
        ...
        <div className="streak-metric-card">
          ...
          <div className="streak-weeks-grid">
            {(() => {
              ...
              const totalAllLessons = completedCount + completedSqlCount + completedFastApiCount + completedExpressCount + completedPythonCount + completedTypescriptCount + completedRQualitativeCount + completedNvivoCount;
      ...
      {/* FULLSCREEN IMMERSIVE NVIVO LESSON READER VIEW */}
      {selectedNvivoLesson && selectedNvivoSection && (
        <div className="reader-overlay-view">
          ...
          <div className="reader-header-bar">
            <div className="reader-left-pane">
              <button
                className="reader-back-btn"
                onClick={() => {
                  setSelectedNvivoLesson(null);
                  setSelectedNvivoSection(null);
                  playSynthesizedSound('reset');
                }}
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              ...
              <span className="reader-module-tag">
                {selectedNvivoSection.sectionTitle}
              </span>
            </div>

            <button
              className={`reader-complete-btn ${completedNvivoLessons.includes(selectedNvivoLesson.id) ? 'completed' : ''}`}
              onClick={() => handleMarkNvivoLessonComplete(selectedNvivoLesson.id)}
            >
              <span className="reader-check-box"></span>
              {completedNvivoLessons.includes(selectedNvivoLesson.id) ? 'Completed' : 'Mark Lesson Completed'}
            </button>
          </div>

          <div className="reader-body-split">
            <article className="reader-scroll-article">
              <h1 className="reader-lesson-heading-title">{selectedNvivoLesson.title}</h1>

              <div
                className="reader-html-render"
                dangerouslySetInnerHTML={{ __html: parseMarkdownToHtml(selectedNvivoLesson.content) }}
              />

              <div className="reader-bottom-nav-row">
                <button className="nav-prev-btn" onClick={() => {
                  const allLessons = nvivoLessonsData.flatMap(s => s.lessons.map(l => ({ lesson: l, section: s })));
                  const currIdx = allLessons.findIndex(item => item.lesson.id === selectedNvivoLesson.id);
                  if (currIdx > 0) {
                    const prev = allLessons[currIdx - 1];
                    setSelectedNvivoLesson(prev.lesson);
                    setSelectedNvivoSection(prev.section);
                    playSynthesizedSound('success');
                  }
                }}>
                  &larr; Previous Lesson
                ...
                <button className="nav-next-btn" onClick={() => {
                  ...
                  if (currIdx < allLessons.length - 1) {
                    const next = allLessons[currIdx + 1];
                    setSelectedNvivoLesson(next.lesson);
                    setSelectedNvivoSection(next.section);
                    playSynthesizedSound('success');
                  } else {
                    handleMarkNvivoLessonComplete(selectedNvivoLesson.id);
                  }
                }}>
                  Next Lesson &rarr;
                </button>
            ...
            <aside className="reader-side-outline-panel">
              <h4>Module Outline</h4>
              <div className="reader-outline-links-list">
                {selectedNvivoSection.lessons.map(les => {
                  const isActive = les.id === selectedNvivoLesson.id;
                  return (
                    <button
                      key={les.id}
                      className={`reader-link-item-btn ${isActive ? 'active' : ''}`}
                      onClick={() => {
                        setSelectedNvivoLesson(les);
                        playSynthesizedSound('success');
                      }}
                    >
                      {les.title}
