...
3. `ContextObject` - The context object is a component that accepts a prop called `value`, which is the context value that’s going to be passed down to the components no matter how deeply they’re nested. In other words, a way to “provide” the context value to these components. Prior to React 19, `ContextObject.Provider` was used instead of `ContextObject`, so wherever you see `Provider` we mean `ContextObject`
